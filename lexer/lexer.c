...
lex (char* src)
{
	...
	for (; *src != '\0'; src++) {
		...
		if ((*src >= '0' && *src <= '9') || *src == '.') {
			...
			res = _lex_new_tk(res, cur_type, buf);
			res = _lex_new_tk(res, TOK_SUB, "-");
			_lex_reset_buf(&cur_type, &buf_len, &buf_pos, &buf);
		} else if (*src == '*') {
			res = _lex_new_tk(res, cur_type, buf);
			res = _lex_new_tk(res, TOK_MUL, "*");
			_lex_reset_buf(&cur_type, &buf_len, &buf_pos, &buf);
		} else if (*src == '/') {
			res = _lex_new_tk(res, cur_type, buf);
			res = _lex_new_tk(res, TOK_DIV, "/");
			_lex_reset_buf(&cur_type, &buf_len, &buf_pos, &buf);
		} else if (*src == '^') {
			res = _lex_new_tk(res, cur_type, buf);
			res = _lex_new_tk(res, TOK_EXP, "^");
			_lex_reset_buf(&cur_type, &buf_len, &buf_pos, &buf);
		} else if (*src == '%') {
			res = _lex_new_tk(res, cur_type, buf);
			res = _lex_new_tk(res, TOK_MOD, "%");
			_lex_reset_buf(&cur_type, &buf_len, &buf_pos, &buf);
		} else if (*src == '(') {
			res = _lex_new_tk(res, cur_type, buf);
			res = _lex_new_tk(res, TOK_LPAREN, "(");
			_lex_reset_buf(&cur_type, &buf_len, &buf_pos, &buf);
		} else if (*src == ')') {
			res = _lex_new_tk(res, cur_type, buf);
			res = _lex_new_tk(res, TOK_RPAREN, ")");
			_lex_reset_buf(&cur_type, &buf_len, &buf_pos, &buf);
		} else {
